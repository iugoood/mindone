...
class DeepseekV2MoEGate(mindspore.nn.Cell):
    def __init__(self, config: DeepseekV2Config):
        ...
        self.weight = mindspore.Parameter(mindspore.mint.empty((self.num_experts, self.gating_dim)))

    def construct(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        batch_size, seq_len, hidden_dim = hidden_states.shape
        ### compute gating score
        hidden_states = hidden_states.view(-1, hidden_dim)
        logits = mindspore.mint.nn.functional.linear(hidden_states.type(mindspore.float32), self.weight.type(mindspore.float32), None)
        ...
        if self.topk_method == "greedy":
            topk_weight, topk_idx = mindspore.mint.topk(scores, k=self.top_k, dim=-1, sorted=False)
        elif self.topk_method == "group_limited_greedy":
            group_scores = scores.view(batch_size * seq_len, self.num_group, -1).max(dim=-1).values  # [n, num_group]
            group_idx = mindspore.mint.topk(group_scores, k=self.topk_group, dim=-1, sorted=False)[1]  # [n, top_k_group]
            group_mask = mindspore.mint.zeros_like(group_scores)  # [n, num_group]
            ...
            topk_weight, topk_idx = mindspore.mint.topk(tmp_scores, k=self.top_k, dim=-1, sorted=False)
...
class DeepseekV2MoE(mindspore.nn.Cell):
    ...
    def moe(self, hidden_states: mindspore.Tensor, topk_ids: mindspore.Tensor, topk_weight: mindspore.Tensor) -> mindspore.Tensor:
        ...
        outs = mindspore.mint.cat(outputs, dim=0) if outputs else sorted_tokens.new_empty(0)

        # Reorder and combine outputs
        new_x = mindspore.mint.empty_like(outs)
        new_x[indicies] = outs
        hidden_states = (
...
class DeepseekV2MLP(mindspore.nn.Cell):
    def __init__(self, config: DeepseekV2Config, hidden_size=None, intermediate_size=None):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size if hidden_size is None else hidden_size
        self.intermediate_size = config.intermediate_size if intermediate_size is None else intermediate_size
        self.gate_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=config.mlp_bias)
        self.up_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=config.mlp_bias)
        self.down_proj = mindspore.mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=config.mlp_bias)
...
class DeepseekV2RMSNorm(mindspore.nn.Cell):
...
class DeepseekV2RotaryEmbedding(mindspore.nn.Cell):
    def __init__(self, config: DeepseekV2Config):
        super().__init__()
    ...
    def construct(self, x, position_ids):
        ...
        freqs_cis = mindspore.mint.polar(mindspore.mint.ones_like(freqs), freqs)  # Convert to complex representation
...
def view_as_complex(input: mindspore.Tensor) -> mindspore.Tensor:
...
def apply_rotary_emb(
    ...
    xq_out = ops.view_as_real(xq_ * freqs_cis).flatten(3).type_as(xq)
    xk_out = ops.view_as_real(xk_ * freqs_cis).flatten(3).type_as(xk)
    return xq_out, xk_out


class DeepseekV2Attention(mindspore.nn.Cell):
    ...
    def __init__(self, config: DeepseekV2Config, layer_idx: Optional[int] = None):
        ...
        if self.q_lora_rank is None:
            self.q_proj = mindspore.mint.nn.Linear(self.hidden_size, self.num_heads * self.qk_head_dim, bias=False)
        else:
            self.q_a_proj = mindspore.mint.nn.Linear(self.hidden_size, config.q_lora_rank, bias=config.attention_bias)
            self.q_a_layernorm = DeepseekV2RMSNorm(config.q_lora_rank)
            self.q_b_proj = mindspore.mint.nn.Linear(config.q_lora_rank, self.num_heads * self.qk_head_dim, bias=False)

        self.kv_a_proj_with_mqa = mindspore.mint.nn.Linear(
            self.hidden_size,
            config.kv_lora_rank + config.qk_rope_head_dim,
            bias=config.attention_bias,
        )
        self.kv_a_layernorm = DeepseekV2RMSNorm(config.kv_lora_rank)
        self.kv_b_proj = mindspore.mint.nn.Linear(
            config.kv_lora_rank,
    ...
    def construct(
        ...
        **kwargs,
        ...
        q_nope, q_pe = mindspore.mint.split(q, [self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)

        compressed_kv = self.kv_a_proj_with_mqa(hidden_states)
        k_nope, k_pe = mindspore.mint.split(compressed_kv, [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
        k_nope = self.kv_b_proj(self.kv_a_layernorm(k_nope)).view(key_shape).transpose(1, 2)
        k_nope, value_states = mindspore.mint.split(k_nope, [self.qk_nope_head_dim, self.v_head_dim], dim=-1)

        k_pe = k_pe.view(batch_size, 1, seq_length, self.qk_rope_head_dim)
        q_pe, k_pe = apply_rotary_emb(q_pe, k_pe, position_embeddings)

        k_pe = k_pe.expand(*k_nope.shape[:-1], -1)
        query_states = mindspore.mint.cat((q_nope, q_pe), dim=-1)
        key_states = mindspore.mint.cat((k_nope, k_pe), dim=-1)
        ...
        if self.config._attn_implementation == "flash_attention_2" and self.qk_head_dim != self.v_head_dim:
            value_states = mindspore.mint.nn.functional.pad(value_states, [0, self.qk_head_dim - self.v_head_dim])
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if inputs_embeds is None:
            inputs_embeds: mindspore.Tensor = self.embed_tokens(input_ids)
