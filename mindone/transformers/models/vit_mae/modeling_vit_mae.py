...
from mindone.models.utils import normal_, xavier_uniform_
...
class ViTMAEModelOutput(ModelOutput):
    ...
    Args:
        ...
        mask (`ms.Tensor` of shape `(batch_size, sequence_length)`):
            Tensor indicating which patches are masked (1) and which are not (0).
        ids_restore (`ms.Tensor` of shape `(batch_size, sequence_length)`):
    ...
    ids_restore: Optional[mindspore.Tensor] = None
...
class ViTMAEDecoderOutput(ModelOutput):
    ...
    Args:
        logits (`ms.Tensor` of shape `(batch_size, sequence_length, patch_size ** 2 * num_channels)`):
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, add_cls_token=False):
    ...
    Returns:
        (`ms.Tensor` of shape (grid_size*grid_size, embed_dim) or (1+grid_size*grid_size, embed_dim): the
...
class ViTMAEEmbeddings(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = mindspore.Parameter(
            mindspore.mint.zeros((1, self.num_patches + 1, config.hidden_size)), requires_grad=False
        )
    ...
    def initialize_weights(self):
        ...
        self.position_embeddings.data.copy_(mindspore.Tensor.from_numpy(pos_embed).float().unsqueeze(0))
        ...
        normal_(self.cls_token, std=self.config.initializer_range)
    ...
    def interpolate_pos_encoding(self, embeddings: mindspore.Tensor, height: int, width: int) -> mindspore.Tensor:
        ...
        images. This method is also adapted to support mindspore.jit tracing.
    ...
    def random_masking(self, sequence, noise=None):
        ...
        Args:
            sequence (`ms.Tensor` of shape `(batch_size, sequence_length, dim)`)
            noise (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*) which is
        ...
        if noise is None:
            noise = mindspore.mint.rand(batch_size, seq_length, )  # noise in [0, 1]

        # sort noise for each sample
        ids_shuffle = mindspore.mint.argsort(noise, dim=1)  # ascend: small is keep, large is remove
        ids_restore = mindspore.mint.argsort(ids_shuffle, dim=1)

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        sequence_unmasked = mindspore.mint.gather(sequence, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, dim))

        # generate the binary mask: 0 is keep, 1 is remove
        mask = mindspore.mint.ones([batch_size, seq_length], )
        mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        mask = mindspore.mint.gather(mask, dim=1, index=ids_restore)

        return sequence_unmasked, mask, ids_restore

    def construct(self, pixel_values, noise=None, interpolate_pos_encoding: bool = False):
        ...
        embeddings = mindspore.mint.cat((cls_tokens, embeddings), dim=1)

        return embeddings, mask, ids_restore


class ViTMAEPatchEmbeddings(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.projection = mindspore.mint.nn.Conv2d(num_channels, hidden_size, kernel_size=patch_size, stride=patch_size)
...
class ViTMAESelfAttention(mindspore.nn.Cell):
...
class ViTMAESelfOutput(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: ViTMAEConfig) -> None:
        ...
        self.dropout = mindspore.mint.nn.Dropout(p=config.hidden_dropout_prob)
...
class ViTMAEAttention(mindspore.nn.Cell):
    def __init__(self, config: ViTMAEConfig) -> None:
        super().__init__()
...
class ViTMAEIntermediate(mindspore.nn.Cell):
    def __init__(self, config: ViTMAEConfig) -> None:
        super().__init__()
...
class ViTMAEOutput(mindspore.nn.Cell):
    def __init__(self, config: ViTMAEConfig) -> None:
        super().__init__()
...
class ViTMAELayer(mindspore.nn.Cell):
...
class ViTMAEEncoder(mindspore.nn.Cell):
    def __init__(self, config: ViTMAEConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([ViTMAELayer(config) for _ in range(config.num_hidden_layers)])
...
VIT_MAE_START_DOCSTRING = r"""
    This model is a Mindspore [mindspore.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) subclass. Use it
...
class ViTMAEModel(ViTMAEPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import ViTMAEModel
...
class ViTMAEDecoder(mindspore.nn.Cell):
    def __init__(self, config, num_patches):
        super().__init__()
        self.decoder_embed = mindspore.mint.nn.Linear(config.hidden_size, config.decoder_hidden_size, bias=True)
        self.mask_token = mindspore.Parameter(mindspore.mint.zeros((1, 1, config.decoder_hidden_size)))
        self.decoder_pos_embed = mindspore.Parameter(
            mindspore.mint.zeros((1, num_patches + 1, config.decoder_hidden_size)), requires_grad=False
        )  # fixed sin-cos embedding
        ...
        self.decoder_layers = mindspore.nn.CellList(
            [ViTMAELayer(decoder_config) for _ in range(config.decoder_num_hidden_layers)]
        )

        self.decoder_norm = mindspore.mint.nn.LayerNorm(config.decoder_hidden_size, eps=config.layer_norm_eps)
        self.decoder_pred = mindspore.mint.nn.Linear(
    ...
    def initialize_weights(self, num_patches):
        ...
        self.decoder_pos_embed.data.copy_(mindspore.Tensor.from_numpy(decoder_pos_embed).float().unsqueeze(0))

        # timm's trunc_normal_(std=.02) is effectively normal_(std=0.02) as cutoff is too big (2.)
        normal_(self.mask_token, std=self.config.initializer_range)
    ...
    def construct(
        ...
        x_ = mindspore.mint.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        # unshuffle
        x_ = mindspore.mint.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))
        x = mindspore.mint.cat([x[:, :1, :], x_], dim=1)  # append cls token
        # add pos embed
        if interpolate_pos_encoding:
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def patchify(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        Returns:
            `ms.Tensor` of shape `(batch_size, num_patches, patch_size**2 * num_channels)`:
        ...
        patchified_pixel_values = mindspore.mint.einsum("nchpwq->nhwpqc", patchified_pixel_values)
    ...
    def unpatchify(self, patchified_pixel_values, original_image_size: Optional[Tuple[int, int]] = None):
        """
        Args:
            patchified_pixel_values (`ms.Tensor` of shape `(batch_size, num_patches, patch_size**2 * num_channels)`:
                Patchified pixel values.
            original_image_size (`Tuple[int, int]`, *optional*):
                Original image size.

        Returns:
            `ms.Tensor` of shape `(batch_size, num_channels, height, width)`:
                Pixel values.
        """
        patch_size, num_channels = self.config.patch_size, self.config.num_channels
        original_image_size = (
        ...
        patchified_pixel_values = mindspore.mint.einsum("nhwpqc->nchpwq", patchified_pixel_values)
    ...
    def forward_loss(self, pixel_values, pred, mask, interpolate_pos_encoding: bool = False):
        ...
        Args:
            pixel_values (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`):
                Pixel values.
            pred (`ms.Tensor` of shape `(batch_size, num_patches, patch_size**2 * num_channels)`:
        ...
        Returns:
            `ms.Tensor`: Pixel reconstruction loss.
        """
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoImageProcessor
        >>> from mindone.transformers import ViTMAEForPreTraining
        >>> from PIL import Image
        >>> import requests
