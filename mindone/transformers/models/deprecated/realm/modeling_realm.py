...
from transformers.models.deprecated.realm.configuration_realm import RealmConfig
...
class RealmEmbedderOutput(ModelOutput):
    ...
    Args:
        projected_score (`ms.Tensor` of shape `(batch_size, config.retriever_proj_size)`):
    ...
    projected_score: Optional[ms.Tensor] = None
...
class RealmScorerOutput(ModelOutput):
    ...
    Args:
        relevance_score (`ms.Tensor` of shape `(batch_size, config.num_candidates)`):
            The relevance score of document candidates (before softmax).
        query_score (`ms.Tensor` of shape `(batch_size, config.retriever_proj_size)`):
            Query score derived from the query embedder.
        candidate_score (`ms.Tensor` of shape `(batch_size, config.num_candidates, config.retriever_proj_size)`):
            Candidate score derived from the embedder.
    """

    relevance_score: Optional[ms.Tensor] = None
    query_score: Optional[ms.Tensor] = None
    candidate_score: Optional[ms.Tensor] = None
...
class RealmReaderOutput(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Total loss.
        retriever_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Retriever loss.
        reader_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Reader loss.
        retriever_correct (`ms.Tensor` of shape `(config.searcher_beam_size,)`, *optional*):
            Whether or not an evidence block contains answer.
        reader_correct (`ms.Tensor` of shape `(config.reader_beam_size, num_candidates)`, *optional*):
            Whether or not a span candidate contains answer.
        block_idx (`ms.Tensor` of shape `()`):
            The index of the retrieved evidence block in which the predicted answer is most likely.
        candidate (`ms.Tensor` of shape `()`):
            The index of the retrieved span candidates in which the predicted answer is most likely.
        start_pos (`ms.Tensor` of shape `()`):
            Predicted answer starting position in *RealmReader*'s inputs.
        end_pos (`ms.Tensor` of shape `()`):
    ...
    retriever_loss: Optional[ms.Tensor] = None
    reader_loss: Optional[ms.Tensor] = None
    retriever_correct: ms.Tensor = None
    reader_correct: ms.Tensor = None
    block_idx: Optional[ms.Tensor] = None
    candidate: Optional[ms.Tensor] = None
    start_pos: ms.int32 = None
    end_pos: ms.int32 = None
    hidden_states: Optional[tuple[ms.Tensor]] = None
    attentions: Optional[tuple[ms.Tensor]] = None
...
class RealmForOpenQAOutput(ModelOutput):
    ...
    Args:
        reader_output (`dict`):
            Reader output.
        predicted_answer_ids (`ms.Tensor` of shape `(answer_sequence_length)`):
            Predicted answer ids.
    """

    reader_output: dict = None
    predicted_answer_ids: Optional[ms.Tensor] = None
...
class RealmScorerProjection(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.predictions = RealmLMPredictionHead(config)
        self.dense = mint.nn.Linear(config.hidden_size, config.retriever_proj_size)
        self.LayerNorm = mint.nn.LayerNorm(config.retriever_proj_size, eps=config.layer_norm_eps)
...
class RealmReaderProjection(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.dense_intermediate = mint.nn.Linear(config.hidden_size, config.span_hidden_size * 2)
        self.dense_output = mint.nn.Linear(config.span_hidden_size, 1)
        self.layer_normalization = mint.nn.LayerNorm(config.span_hidden_size, eps=config.reader_layer_norm_eps)
        self.relu = nn.ReLU()
    ...
    def construct(self, hidden_states, block_mask):
        def span_candidates(masks):
            ...
            def _spans_given_width(width):
                current_starts = mint.arange(max_sequence_len - width + 1)
                current_ends = mint.arange(width - 1, max_sequence_len)
                return current_starts, current_ends

            starts, ends = zip(*(_spans_given_width(w + 1) for w in range(self.config.max_span_width)))

            # [num_spans]
            starts = mint.cat(starts, 0)
            ends = mint.cat(ends, 0)

            # [num_retrievals, num_spans]
            start_masks = mint.index_select(masks, dim=-1, index=starts)
            end_masks = mint.index_select(masks, dim=-1, index=ends)
            span_masks = start_masks * end_masks

            return starts, ends, span_masks

        def mask_to_score(mask, dtype=ms.float32):
            return (1.0 - mask.type(dtype)) * dtype_to_min(dtype)
        ...
        candidate_start_projections = mint.index_select(start_projection, dim=1, index=candidate_starts)
        candidate_end_projections = mint.index_select(end_projection, dim=1, index=candidate_ends)
...
REALM_START_DOCSTRING = r"""
    This model is a Mindspore [torch.nn.Cell](https://pytorch.org/docs/stable/nn.html#torch.nn.Cell) sub-class. Use
...
class RealmScorer(RealmPreTrainedModel):
    ...
    def construct(
        ...
        token_type_ids: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        candidate_input_ids: Optional[ms.Tensor] = None,
        candidate_attention_mask: Optional[ms.Tensor] = None,
        candidate_token_type_ids: Optional[ms.Tensor] = None,
        candidate_inputs_embeds: Optional[ms.Tensor] = None,
        head_mask: Optional[ms.Tensor] = None,
        inputs_embeds: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        r"""
        candidate_input_ids (`ms.Tensor` of shape `(batch_size, num_candidates, sequence_length)`):
            ...
            [What are input IDs?](../glossary#input-ids)
        candidate_attention_mask (`ms.Tensor` of shape `(batch_size, num_candidates, sequence_length)`, *optional*):
            Mask to avoid performing attention on padding token indices. Mask values selected in `[0, 1]`:
            ...
            [What are attention masks?](../glossary#attention-mask)
        candidate_token_type_ids (`ms.Tensor` of shape `(batch_size, num_candidates, sequence_length)`, *optional*):
            Segment token indices to indicate first and second portions of the inputs. Indices are selected in `[0,
            1]`:
            ...
            [What are token type IDs?](../glossary#token-type-ids)
        candidate_inputs_embeds (`ms.Tensor` of shape `(batch_size * num_candidates, sequence_length, hidden_size)`, *optional*):
        ...
        relevance_score = mint.einsum("bd,bnd->bn", query_score, candidate_score)
...
class RealmKnowledgeAugEncoder(RealmPreTrainedModel):
    ...
    def construct(
        ...
        mlm_mask: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, MaskedLMOutput]:
        r"""
        relevance_score (`ms.Tensor` of shape `(batch_size, num_candidates)`, *optional*):
        ...
        mlm_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        if labels is not None:
            ...
            if mlm_mask is None:
                mlm_mask = mint.ones_like(labels, dtype=ms.float32)
            else:
                mlm_mask = mlm_mask.type(ms.float32)
            ...
            # []
            masked_lm_loss = -mint.nansum(mint.sum(marginal_gold_log_probs * mlm_mask) / mint.sum(mlm_mask))
...
class RealmReader(RealmPreTrainedModel):
    ...
    def construct(
        ...
        inputs_embeds: Optional[ms.Tensor] = None,
        relevance_score: Optional[ms.Tensor] = None,
        block_mask: Optional[ms.Tensor] = None,
        start_positions: Optional[ms.Tensor] = None,
        end_positions: Optional[ms.Tensor] = None,
        has_answers: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, RealmReaderOutput]:
        r"""
        relevance_score (`ms.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Relevance score, which must be specified if you want to compute the logits and marginal log loss.
        block_mask (`ms.Tensor` of shape `(searcher_beam_size, sequence_length)`, *optional*):
            The mask of the evidence block, which must be specified if you want to compute the logits and marginal log
            loss.
        start_positions (`ms.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        end_positions (`ms.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Labels for position (index) of the end of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        has_answers (`ms.Tensor` of shape `(searcher_beam_size,)`, *optional*):
        ...
        retriever_logits = mint.unsqueeze(relevance_score[0 : self.config.reader_beam_size], -1)
        # [reader_beam_size, num_candidates]
        reader_logits += retriever_logits
        # []
        predicted_block_index = mint.argmax(mint.max(reader_logits, dim=1).values)
        # []
        predicted_candidate = mint.argmax(mint.max(reader_logits, dim=0).values)
        # [1]
        predicted_start = mint.index_select(candidate_starts, dim=0, index=predicted_candidate)
        # [1]
        predicted_end = mint.index_select(candidate_ends, dim=0, index=predicted_candidate)
        ...
        if start_positions is not None and end_positions is not None and has_answers is not None:
            ...
            def compute_correct_candidates(candidate_starts, candidate_ends, gold_starts, gold_ends):
                ...
                is_gold_start = mint.eq(
                    mint.unsqueeze(mint.unsqueeze(candidate_starts, 0), 0), mint.unsqueeze(gold_starts, -1)
                )
                is_gold_end = mint.eq(
                    mint.unsqueeze(mint.unsqueeze(candidate_ends, 0), 0), mint.unsqueeze(gold_ends, -1)
                )

                # [reader_beam_size, num_candidates]
                return mint.any(mint.logical_and(is_gold_start, is_gold_end), 1)
            ...
            def marginal_log_loss(logits, is_correct):
                ...
                # []
                log_numerator = mint.logsumexp(logits + mask_to_score(is_correct, dtype=logits.dtype), dim=-1)
                log_denominator = mint.logsumexp(logits, dim=-1)
            ...
            any_retriever_correct = mint.any(retriever_correct)
            ...
            any_reader_correct = mint.any(reader_correct)
            ...
            retriever_loss *= any_retriever_correct.type(ms.float32)
            reader_loss *= any_reader_correct.type(ms.float32)
...
class RealmForOpenQA(RealmPreTrainedModel):
    ...
    def construct(
        ...
        attention_mask: Optional[ms.Tensor] = None,
        token_type_ids: Optional[ms.Tensor] = None,
        answer_ids: Optional[ms.Tensor] = None,
        return_dict: Optional[bool] = None,
        ...
        # [1, block_emb_size]
        batch_scores = mint.einsum("BD,QD->QB", self.block_emb, question_projection)
        # [1, searcher_beam_size]
        _, retrieved_block_ids = mint.topk(batch_scores, k=self.searcher_beam_size, dim=-1)
        # [searcher_beam_size]
        retrieved_block_ids = retrieved_block_ids.squeeze()
        # [searcher_beam_size, projection_size]
        retrieved_block_emb = mint.index_select(self.block_emb, dim=0, index=retrieved_block_ids)
        ...
        block_mask = concat_inputs.special_tokens_mask.type(ms.bool_)
        block_mask.logical_not_().logical_and_(concat_inputs.token_type_ids.type(ms.bool_))

        if has_answers is not None:
            has_answers = ms.tensor(has_answers, dtype=ms.bool_)
            start_pos = ms.tensor(start_pos, dtype=ms.int32)
            end_pos = ms.tensor(end_pos, dtype=ms.int32)

        # [searcher_beam_size]
        retrieved_logits = mint.einsum("D,BD->B", question_projection.squeeze(), retrieved_block_emb)
