...
class NemotronLayerNorm1P(mint.nn.LayerNorm):
    def __init__(
        self,
        ...
        super().__init__(normalized_shape, eps, elementwise_affine, bias, dtype)
...
class NemotronRotaryEmbedding(nn.Cell):
    # Ignore copy
    def __init__(
        self,
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    return mint.cat((q_embed, q_pass), dim=-1), mint.cat((k_embed, k_pass), dim=-1)


class NemotronMLP(nn.Cell):
...
class NemotronAttention(nn.Cell):
    ...
    def __init__(self, config: NemotronConfig, layer_idx: Optional[int] = None):
        ...
        self.o_proj = mint.nn.Linear(self.head_dim * self.num_heads, self.hidden_size, bias=config.attention_bias)
...
class NemotronSdpaAttention(NemotronAttention):
    ...
    `NemotronAttention` as the weights of the module stays untouched. The only changes are on the construct pass to adapt to
    SDPA API.
    """
...
class NemotronDecoderLayer(nn.Cell):
    # Ignore copy
