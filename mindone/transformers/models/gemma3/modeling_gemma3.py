...
class Gemma3CausalLMOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        image_hidden_states (`ms.Tensor`, *optional*):
            A `ms.Tensor` of size `(batch_size, sequence_length, hidden_size)`.
        ...
        if self.config._attn_implementation != "eager":
            ...
            if self.config._attn_implementation == "flash_attention_2":
                attention_mask = ms.Tensor(
                    np.triu(np.ones(shape=(seq_len, seq_len), dtype=np.float16), 1), dtype=ms.uint8
                )
...
class Gemma3DecoderLayer(nn.Cell):
    def __init__(self, config: Gemma3TextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def construct(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if return_dict:
            ...
            output = tuple([hidden_states, past_key_values, all_hidden_states, all_self_attns])
        return output
...
class Gemma3ForConditionalGeneration(Gemma3PreTrainedModel, GenerationMixin):
    ...
    def _update_causal_mask(
        ...
        if token_type_ids is not None and sequence_length != 1:
            ...
            token_type_mask = token_type_mask.unsqueeze(1).to(dtype=ms.bool_)
