...
class PersimmonAttention(nn.Cell):
    ...
    def construct(
        ...
        attn_weights = mint.softmax(attn_weights, dtype=ms.float32, dim=-1).to(query_states.dtype)
...
class PersimmonDecoderLayer(nn.Cell):
    ...
    def construct(
        ...
        Args:
            ...
            past_key_value (`Tuple(ms.Tensor)`, *optional*):
...
class PersimmonForSequenceClassification(PersimmonPreTrainedModel):
    ...
    def construct(
        ...
        if self.config.pad_token_id is None:
            ...
            non_pad_mask = (input_ids != self.config.pad_token_id)
