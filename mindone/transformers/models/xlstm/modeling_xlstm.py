...
from transformers.models.xlstm.configuration_xlstm import xLSTMConfig, round_up_to_next_multiple_of


from functools import partial
from typing import Callable, Literal

mLSTMLayerStateType = tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]
mLSTMStateType = dict[int, mLSTMLayerStateType]

external_xlstm = False

def soft_cap(values: mindspore.Tensor, cap_value: Optional[Union[float, mindspore.Tensor]] = None) -> mindspore.Tensor:
    """
    ...
    if cap_value is None:
        return values
    return cap_value * mindspore.mint.tanh(values / cap_value)

def mlstm_chunkwise_recurrent_fw_C(
    matK: mindspore.Tensor,
    matV: mindspore.Tensor,
    vecB: mindspore.Tensor,
    vecI: mindspore.Tensor,
    matC_states: Optional[mindspore.Tensor] = None,
    vecN_states: Optional[mindspore.Tensor] = None,
    scaMinter_states: Optional[mindspore.Tensor] = None,
    matC_initial: Optional[mindspore.Tensor] = None,
    vecN_initial: Optional[mindspore.Tensor] = None,
    scaMinter_initial: Optional[mindspore.Tensor] = None,
    ...
    if matC_states is None:
        matC_states = mindspore.mint.zeros((batch_size, nh, (nc + 1) * dhqk, dhhv), dtype=_dtype, )
    if vecN_states is None:
        vecN_states = mindspore.mint.zeros((batch_size, nh, (nc + 1) * dhqk), dtype=_dtype, )
    if scaMinter_states is None:
        scaMinter_states = mindspore.mint.zeros((batch_size, nh, (nc + 1)), dtype=_dtype, )

    # assign the initial states to the running states
    matC_k = (
        mindspore.mint.zeros((batch_size, nh, dhqk, dhhv), dtype=_dtype, )
        if matC_initial is None
        else matC_initial
    )
    vecN_k = (
        mindspore.mint.zeros((batch_size, nh, dhqk), dtype=_dtype, ) if vecN_initial is None else vecN_initial
    )
    scaM_inter_k = (
        mindspore.mint.zeros((batch_size, nh, 1), dtype=_dtype, )
    ...
    for key in range(0, num_chunks):
        ...
        scaG_k = scaG[:, :, key]
        scaM_inter_k_next = mindspore.mint.max(scaG_k + scaM_inter_k, scaA_max_k)
        # C_k update
        matK_chunk = matK[:, :, key * chunk_size : (key + 1) * chunk_size, :]  # * qk_scale
        matV_chunk = matV[:, :, key * chunk_size : (key + 1) * chunk_size, :]
        vecA_k = vecA[:, :, key, :]

        vecAbar_k = mindspore.mint.exp(vecA_k - scaM_inter_k_next[..., None])[:, :, :, None]

        matK_chunk_gated = matK_chunk * vecAbar_k

        scaGbar_k = mindspore.mint.exp(scaG_k + scaM_inter_k - scaM_inter_k_next)[:, :, None]
...
def mlstm_chunkwise_parallel_fw_H(
    matQ: mindspore.Tensor,
    matK: mindspore.Tensor,
    matV: mindspore.Tensor,
    # these states must be all states up to the last chunk, i.e. :-1
    matC_states: mindspore.Tensor,
    vecN_states: mindspore.Tensor,
    scaMinter_states: mindspore.Tensor,
    vecI: mindspore.Tensor,
    vecB: mindspore.Tensor,
    qk_scale: float,
    chunk_size: int = 64,
    ...
    matF_logsig_mask_chunk = mindspore.mint.where(ltr, matF_logsig_chunk, -float("inf"))

    matLogD_chunk = matF_logsig_mask_chunk + vecI[:, :, :, None, :]

    # max_state intra
    vecMintra_k = mindspore.mint.max(matLogD_chunk, dim=-1, keepdim=False).values

    # max_state combined
    vecM_b_inter = vecB + scaMinter_k_states[:, :, :, None]
    vecM_k_combine = mindspore.mint.maximum(vecM_b_inter, vecMintra_k)

    vecM_k_combine = vecM_k_combine[:, :, :, :, None]
    vecM_b_inter = vecM_b_inter[:, :, :, :, None]

    matLogD_stabilized_chunk = matLogD_chunk - vecM_k_combine
    matD_chunk = mindspore.mint.exp(matLogD_stabilized_chunk)
    ...
    vecBbar = mindspore.mint.exp(vecM_b_inter - vecM_k_combine)
    ...
    vecDenom_max_common = mindspore.mint.maximum(mindspore.mint.abs(vecDenom_l_common), mindspore.mint.exp(-vecM_k_combine))
...
def mlstm_chunkwise_fw(
    ...
    igate: mindspore.Tensor,
    fgate: mindspore.Tensor,
    cstate: Optional[mindspore.Tensor] = None,
    nstate: Optional[mindspore.Tensor] = None,
    mstate: Optional[mindspore.Tensor] = None,
    qk_scale: Optional[float] = None,
    ...
    mindspore.Tensor,
    mindspore.Tensor,
    Optional[tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]],
    Optional[tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]],
]:
...
def mlstm_chunkwise_native_autograd(
    ...
    igate: mindspore.Tensor,
    fgate: mindspore.Tensor,
    c_initial: Optional[mindspore.Tensor] = None,
    n_initial: Optional[mindspore.Tensor] = None,
    m_initial: Optional[mindspore.Tensor] = None,
    return_last_states: bool = False,
    eps: float = 1e-6,
    chunk_size: int = 64,
    **kwargs,
) -> Union[mindspore.Tensor, tuple[mindspore.Tensor, tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]]]:
    batch_size, nh, sequence_length, dhqk = query.shape
    if sequence_length % chunk_size != 0:
        raise ValueError(f"Sequence length {sequence_length} is not divisible by chunk size {chunk_size}.")
    nc = sequence_length // chunk_size
    ...
    # compute the gates, the g and the a and b vectors
    vecF_logsig = mindspore.mint.nn.functional.logsigmoid(vecF)
    vecB = vecF_logsig.cumsum(-1)
...
def mlstm_recurrent_step_native(
    ...
    igate: mindspore.Tensor,
    fgate: mindspore.Tensor,
    cstate: mindspore.Tensor,
    nstate: mindspore.Tensor,
    mstate: mindspore.Tensor,
    eps: float = 1e-6,
    dtype_state: mindspore.dtype = mindspore.float32,
    **kwargs,
) -> tuple[mindspore.Tensor, tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]]:
    ...
    # gates
    scaF_log = mindspore.mint.nn.functional.logsigmoid(fgate)

    # update rule
    scaM_state_new = mindspore.mint.max(scaF_log + scaM_old, igate)

    scaF_act = mindspore.mint.exp(scaF_log + scaM_old - scaM_state_new)
    scaI_act = mindspore.mint.exp(igate - scaM_state_new)
    ...
    max_val = mindspore.mint.exp(-scaM_state_new)
    h_denom = (mindspore.mint.maximum(qn_dotproduct.abs(), max_val) + eps).to(dtype=dtype_state)
    h = h_num / h_denom
...
def mlstm_recurrent_sequence_native(
    ...
    **kwargs,
    ...
    if c_initial is not None:
        ...
        # memory state
        matC_state = mindspore.mint.zeros((batch_size, nh, dhqk, dhv), dtype=dtype_state, )
        # normalizer state
        vecN_state = mindspore.mint.zeros((batch_size, nh, dhqk), dtype=dtype_state, )
        # max state
        vecM_state = mindspore.mint.zeros((batch_size, nh, 1), dtype=dtype_state, )
    ...
    matH = mindspore.mint.stack(vecH_list, dim=-2)
...
def wrap_chunkwise_pad_zeros(
    ...
    return_last_states: bool = False,
    eps: float = 1e-6,
    autocast_kernel_dtype: mindspore.dtype = mindspore.bfloat16,
    chunk_size: int = 64,
    **kwargs,
) -> Union[mindspore.Tensor, tuple[mindspore.Tensor, tuple[mindspore.Tensor, mindspore.Tensor, mindspore.Tensor]]]:
    if return_last_states:
...
def wrap_chunkwise_arbitrary_sequence_length(
    ...
    in a single step and loop over this in mindspore.
    ...
    c_state = (
        c_initial
        if c_initial is not None
        else mindspore.mint.zeros(batch_size, nh, dhqk, dhhv, dtype=mindspore.float32)
    )
    n_state = (
        n_initial
        if n_initial is not None
        else mindspore.mint.zeros(batch_size, nh, dhqk, dtype=mindspore.float32)
    )
    m_state = (
        m_initial
        if m_initial is not None
        else mindspore.mint.zeros(batch_size, nh, 1, dtype=mindspore.float32)
    )
    ...
    if sequence_length > 1:
        ...
        if remaining_seq_len > 0:
            ...
            h_outs.append(h_out)
        h_out = mindspore.mint.cat(h_outs, dim=2)
...
class xLSTMBackend(mindspore.nn.Cell):
    """xLSTM Backend Module for Mindspore.
...
class xLSTMRMSNorm(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        if use_weight:
            self.weight = mindspore.Parameter(mindspore.mint.ones(num_features))
        else:
            self.weight = None

        if use_bias:
            self.bias = mindspore.Parameter(mindspore.mint.zeros(num_features))
        else:
            self.bias = None

    def _apply_weight_bias(self, x: mindspore.Tensor) -> mindspore.Tensor:
    ...
    def _rms_normalize(self, x: mindspore.Tensor) -> mindspore.Tensor:
        # apply rms norm over the last dimension, i.e. HD dimension
        in_dtype = x.dtype
        if self.force_float32_reductions:
            x = x.float()
        x = x * mindspore.mint.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)
...
class xLSTMMultiHeadLayerNorm(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        if use_weight:
            self.weight = mindspore.Parameter(mindspore.mint.ones(self.num_features))
        else:
            self.weight = None

        if use_bias:
            self.bias = mindspore.Parameter(mindspore.mint.zeros(self.num_features))
        else:
            self.bias = None
    ...
    def _layer_normalize(self, x: mindspore.Tensor) -> mindspore.Tensor:
        # apply layer norm over the last dimension, i.e. HD dimension
        in_dtype = x.dtype
        if self.force_float32_reductions:
            x = x.float()
        x_centered = x - x.mean(dim=-1, keepdim=True)
        y = x_centered * mindspore.mint.rsqrt(x.var(dim=-1, keepdim=True, unbiased=False) + self.eps)
...
class xLSTMFeedForward(mindspore.nn.Cell):
    def __init__(self, config: xLSTMConfig):
        super().__init__()
        self.config = config
        ...
        if self.config.weight_mode == "single":
            self.proj_up_gate = mindspore.mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=self.up_proj_dim,
                bias=self.config.use_bias,
            )
            self.proj_up = mindspore.mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=self.up_proj_dim,
                bias=self.config.use_bias,
            )
        elif self.config.weight_mode == "fused":
            self.proj_up_gate_z = mindspore.mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=2 * self.up_proj_dim,
                bias=self.config.use_bias,
            )

        self.proj_down = mindspore.mint.nn.Linear(
            in_features=self.up_proj_dim,
            out_features=config.hidden_size,
            bias=self.config.use_bias,
        )

        self.act_fn = mindspore.mint.nn.SiLU()
...
class xLSTMLayer(mindspore.nn.Cell):
    def __init__(self, config: xLSTMConfig):
        super().__init__()
        self.config = config
        ...
        if self.config.weight_mode == "single":
            self.q = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.qk_dim,
                bias=self.config.use_bias,
            )
            self.k = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.qk_dim,
                bias=self.config.use_bias,
            )
            self.v = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.v_dim,
                bias=self.config.use_bias,
            )

            self.ogate_preact = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.v_dim,
                bias=self.config.use_bias,
            )
            self.igate_preact = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.config.num_heads,
                bias=True,
            )
            self.fgate_preact = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.config.num_heads,
                bias=True,
            )
        elif self.config.weight_mode == "fused":
            self.qkv_opreact = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=2 * self.qk_dim + 2 * self.v_dim,
                bias=self.config.use_bias,
            )
            self.ifgate_preact = mindspore.mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=2 * self.config.num_heads,
                bias=True,
            )

        self.ogate_act_fn = mindspore.mint.nn.Sigmoid()
    ...
    def construct(
        self, x: mindspore.Tensor, state: Optional[mLSTMLayerStateType] = None
    ) -> tuple[mindspore.Tensor, Optional[mLSTMLayerStateType]]:
        if x.ndim != 3:
...
class xLSTMBlock(mindspore.nn.Cell):
    def __init__(self, config: xLSTMConfig):
        super().__init__()
        self.config = config
    ...
    def construct(
        self, x: mindspore.Tensor, state: Optional[mLSTMStateType] = None
    ) -> tuple[mindspore.Tensor, mLSTMStateType]:
...
class xLSTMPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        if isinstance(module, mindspore.mint.nn.Embedding):
            ...
            if self.config.weight_mode == "single" and "gate" in self._module_name_map(module):
                ...
                with mindspore._no_grad():
                    if "igate" in self._module_name_map(module):
                        module.bias.copy_(-10.0 * mindspore.mint.ones_like(module.bias))
                ...
                with mindspore._no_grad():
                    module.bias[: self.config.num_heads] += -module.bias[
                        : self.config.num_heads
                    ] - 10.0 * mindspore.mint.ones_like(module.bias)
                    module.bias[: self.config.num_heads] += -module.bias[self.config.num_heads :] + mindspore.mint.linspace(
                        3.0,
                        6.0,
                        module.bias.shape[-1],
                    ).to(
...
class xLSTMCache:
        ...
        **kwargs,
        ...
        self.rnn_state = {
            layer: (
                ...
                mindspore.mint.zeros([max_batch_size, config.num_heads, config.qk_head_dim], dtype=dtype, ),
                mindspore.mint.zeros([max_batch_size, config.num_heads, 1], dtype=dtype, ),
            )
    ...
    def reset(self):
        self.rnn_state = {
            layer: (
                mindspore.mint.zeros_like(self.rnn_state[layer][0]),
                mindspore.mint.zeros_like(self.rnn_state[layer][1]),
                mindspore.mint.zeros_like(self.rnn_state[layer][2]),
            )
...
class xLSTMOutput(ModelOutput):
    ...
    last_hidden_state: Optional[mindspore.Tensor]
...
class xLSTMModel(xLSTMPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = mindspore.mint.nn.Embedding(config.vocab_size, config.embedding_dim)
        self.blocks = mindspore.nn.CellList([xLSTMBlock(config) for _ in range(config.num_blocks)])
        ...
        **kwargs,
        ...
        if (
            ...
            with mindspore._no_grad():
                ...
                final_state = mindspore.mint.zeros_like(hidden_states)
