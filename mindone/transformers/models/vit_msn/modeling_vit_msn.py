...
from transformers.models.vit_msn.configuration_vit_msn import ViTMSNConfig
...
class ViTMSNEmbeddings(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: ViTMSNConfig, use_mask_token: bool = False) -> None:
        ...
        self.mask_token = mindspore.Parameter(mindspore.mint.zeros(1, 1, config.hidden_size)) if use_mask_token else None
...
class ViTMSNPatchEmbeddings(mindspore.nn.Cell):
    """
...
class ViTMSNSelfAttention(mindspore.nn.Cell):
...
class ViTMSNSelfOutput(mindspore.nn.Cell):
    """
...
class ViTMSNAttention(mindspore.nn.Cell):
    def __init__(self, config: ViTMSNConfig) -> None:
        super().__init__()
...
class ViTMSNIntermediate(mindspore.nn.Cell):
    def __init__(self, config: ViTMSNConfig) -> None:
        super().__init__()
...
class ViTMSNOutput(mindspore.nn.Cell):
    def __init__(self, config: ViTMSNConfig) -> None:
        super().__init__()
...
class ViTMSNLayer(mindspore.nn.Cell):
...
class ViTMSNEncoder(mindspore.nn.Cell):
    def __init__(self, config: ViTMSNConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([ViTMSNLayer(config) for _ in range(config.num_hidden_layers)])
...
class ViTMSNForImageClassification(ViTMSNPreTrainedModel):
    def __init__(self, config: ViTMSNConfig) -> None:
        ...
        # Classifier head
        self.classifier = mindspore.mint.nn.Linear(config.hidden_size, config.num_labels) if config.num_labels > 0 else mindspore.mint.nn.Identity()
