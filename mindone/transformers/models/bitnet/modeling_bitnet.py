...
class BitNetRMSNorm(mindspore.nn.Cell):
...
class BitNetMLP(mindspore.nn.Cell):
...
class BitNetAttention(mindspore.nn.Cell):
...
class BitNetRotaryEmbedding(mindspore.nn.Cell):
    def __init__(self, config: BitNetConfig):
        super().__init__()
