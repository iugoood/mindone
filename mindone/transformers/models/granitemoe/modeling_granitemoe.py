...
class GraniteMoeRMSNorm(nn.Cell):
...
class GraniteMoeParallelExperts(nn.Cell):
    def __init__(self, num_experts: int, input_size: int, output_size: int) -> None:
        ...
        """
        super().__init__()
        self.weight = ms.Parameter(mint.empty(num_experts, output_size, input_size))
    ...
    def construct(self, inputs, expert_size):
        """
        construct pass of the GraniteMoeParallelExperts module.
        Args:
            inputs (Tensor):
                Input tensor.
            expert_size:
        ...
        for i in range(self.num_experts):
            output_list.append(mint.nn.functional.linear(input_list[i], self.weight[i]))
        results = mint.cat(output_list, dim=0)
        return results


# Copied from transformers.models.jetmoe.modeling_jetmoe.JetMoeTopKGating with JetMoe->GraniteMoe
class GraniteMoeTopKGating(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        top_k_gates = mint.softmax(top_k_logits, dim=1).type_as(hidden_states)  # [num_tokens, top_k]

        # compute number of input given to each expert
        zeros = mint.zeros(
            [top_k_gates.size(0), self.num_experts], dtype=top_k_gates.dtype
...
class GraniteMoeMoE(nn.Cell):
    """
    A Sparsely gated mixture of experts layer with 1-layer Feed-construct networks as experts.
    ...
    def construct(self, layer_input):
        """
        construct pass of the mixture of experts layer.
        ...
        zeros = mint.zeros((bsz * length, self.input_size), dtype=expert_outputs.dtype)
...
class GraniteMoeAttention(nn.Cell):
...
class GraniteMoeDecoderLayer(nn.Cell):
