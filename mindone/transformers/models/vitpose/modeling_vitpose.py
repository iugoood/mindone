...
class VitPoseEstimatorOutput(ModelOutput):
    ...
    heatmaps: Optional[mindspore.Tensor] = None
...
VITPOSE_START_DOCSTRING = r"""
    This model is a Mindspore [mindspore.nn.Module] subclass. Use it
...
class VitPoseSimpleDecoder(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config) -> None:
        ...
        self.upsampling = mindspore.mint.nn.Upsample(scale_factor=config.scale_factor, mode="bilinear", align_corners=False)
        self.conv = mindspore.mint.nn.Conv2d(
            config.backbone_config.hidden_size, config.num_labels, kernel_size=3, stride=1, padding=1
        )

    def construct(self, hidden_state: mindspore.Tensor, flip_pairs: Optional[mindspore.Tensor] = None) -> mindspore.Tensor:
...
class VitPoseClassicDecoder(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: VitPoseConfig):
        ...
        self.deconv1 = mindspore.mint.nn.ConvTranspose2d(
            config.backbone_config.hidden_size, 256, kernel_size=4, stride=2, padding=1, bias=False
        )
        self.batchnorm1 = mindspore.mint.nn.BatchNorm2d(256)
        self.relu1 = mindspore.mint.nn.ReLU()

        self.deconv2 = mindspore.mint.nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1, bias=False)
        self.batchnorm2 = mindspore.mint.nn.BatchNorm2d(256)
        self.relu2 = mindspore.mint.nn.ReLU()

        self.conv = mindspore.mint.nn.Conv2d(256, config.num_labels, kernel_size=1, stride=1, padding=0)

    def construct(self, hidden_state: mindspore.Tensor, flip_pairs: Optional[mindspore.Tensor] = None):
...
class VitPoseForPoseEstimation(VitPosePreTrainedModel):
    ...
    def construct(
        ...
        flip_pairs: Optional[mindspore.Tensor] = None,
