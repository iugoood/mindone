...
"""Mindspore VitPose backbone model.
...
from transformers.models.vitpose_backbone.configuration_vitpose_backbone import VitPoseBackboneConfig
...
class VitPoseBackbonePatchEmbeddings(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.projection = mindspore.mint.nn.Conv2d(num_channels, embed_dim, kernel_size=patch_size, stride=patch_size, padding=2)
...
class VitPoseBackboneEmbeddings(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        ...
        self.position_embeddings = mindspore.Parameter(mindspore.mint.zeros(1, num_patches + 1, config.hidden_size))
...
class VitPoseBackboneSelfAttention(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
...
class VitPoseBackboneSelfOutput(mindspore.nn.Cell):
    """
...
class VitPoseBackboneAttention(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
...
class VitPoseBackboneMoeMLP(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig):
        ...
        self.fc1 = mindspore.mint.nn.Linear(in_features, hidden_features)
        self.act = ACT2FN[config.hidden_act]
        self.fc2 = mindspore.mint.nn.Linear(hidden_features, out_features - part_features)
        self.drop = mindspore.mint.nn.Dropout(p=config.hidden_dropout_prob)

        self.num_experts = num_experts
        experts = [mindspore.mint.nn.Linear(hidden_features, part_features) for _ in range(num_experts)]
        self.experts = mindspore.nn.CellList(experts)

    def construct(self, hidden_state: mindspore.Tensor, indices: mindspore.Tensor) -> mindspore.Tensor:
        expert_hidden_state = mindspore.mint.zeros_like(hidden_state[:, :, -self.part_features :])
        ...
        hidden_state = mindspore.mint.cat([shared_hidden_state, expert_hidden_state], dim=-1)

        return hidden_state


class VitPoseBackboneMLP(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
        in_features = out_features = config.hidden_size
        hidden_features = int(config.hidden_size * config.mlp_ratio)
        self.fc1 = mindspore.mint.nn.Linear(in_features, hidden_features, bias=True)
        self.activation = ACT2FN[config.hidden_act]
        self.fc2 = mindspore.mint.nn.Linear(hidden_features, out_features, bias=True)
...
class VitPoseBackboneLayer(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
    ...
    def construct(
        self,
        hidden_states: mindspore.Tensor,
        dataset_index: Optional[mindspore.Tensor] = None,
        head_mask: Optional[mindspore.Tensor] = None,
        output_attentions: bool = False,
    ) -> Union[Tuple[mindspore.Tensor, mindspore.Tensor], Tuple[mindspore.Tensor]]:
...
class VitPoseBackboneEncoder(mindspore.nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([VitPoseBackboneLayer(config) for _ in range(config.num_hidden_layers)])
...
class VitPoseBackbonePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[mindspore.mint.nn.Linear, mindspore.mint.nn.Conv2d, mindspore.mint.nn.LayerNorm, VitPoseBackboneEmbeddings]) -> None:
        ...
        if isinstance(module, (mindspore.mint.nn.Linear, mindspore.mint.nn.Conv2d)):
            ...
            module.position_embeddings.data = trunc_normal_(
                module.position_embeddings.data.to(mindspore.float32),
                mean=0.0,
...
VITPOSE_BACKBONE_START_DOCSTRING = r"""
    This model is a Mindspore [nn.Cell] subclass. Use it
...
class VitPoseBackbone(VitPoseBackbonePreTrainedModel, BackboneMixin):
    ...
    def construct(
        ...
        >>> pixel_values = ms.mint.randn(1, 3, 256, 192)
        >>> dataset_index = ms.Tensor([1])
