...
class ApplyRotaryEmbUnpad(ms.grad):
    @staticmethod
    def construct(
        ctx,
        qkv,
...
class ModernBertUnpaddedRotaryEmbedding(nn.Cell):
    """
    ...
    def __init__(
        ...
        max_seqlen: if max_seqlen, device, and dtype are provided, we precompute the cos_sin_cache
            ...
            the cos_sin_cache wll be recomputed during the construct pass.
        """
        super().__init__()
        self.dim = dim
        ...
        scale = (
            (ops.arange(0, self.dim, 2, dtype=ms.float32) + 0.4 * dim) / (1.4 * dim)
            if self.scale_base is not None
            else None
        )
        self.register_buffer("scale", inv_freq, persistent=False)
        ...
        if max_seqlen is not None and dtype is not None:
            self._update_cos_sin_cache(max_seqlen, dtype=dtype)

    def _compute_inv_freq(self):
        return 1.0 / (
                self.base
                ** (ops.arange(0, self.dim, 2, dtype=ms.float32) / self.dim)
        )
    ...
    def _update_cos_sin_cache(self, seqlen, dtype=None):
        ...
        if (
            ...
            if self.scale is None:
                ...
                power = (
                    ops.arange(seqlen, dtype=self.scale.dtype)
                    - seqlen // 2
                ) / self.scale_base
...
class ModernBertMLP(nn.Cell):
    ...
    def __init__(self, config: ModernBertConfig):
        super().__init__()
        self.config = config
        self.Wi = nn.Dense(config.hidden_size, int(config.intermediate_size) * 2, has_bias=config.mlp_bias)
        self.act = ACT2FN[config.hidden_activation]
        self.drop = nn.Dropout(p=config.mlp_dropout)
        self.Wo = nn.Dense(config.intermediate_size, config.hidden_size, has_bias=config.mlp_bias)
...
def eager_attention_construct(
    ...
    **_kwargs,
    ...
    attn_weights = mint.matmul(query, key.transpose(2, 3)) * scale
    ...
    attn_weights = ops.dropout(attn_weights, p=module.attention_dropout, training=module.training)
...
def flash_attention_construct(
    ...
    bs: int,
    dim: int,
    target_dtype: ms.dtype = ms.bfloat16,
    **_kwargs,
) -> Tuple[ms.Tensor]:
    # (total_seqlen, 3, nheads, headdim)
    qkv = rotary_emb(qkv, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)

    convert_dtype = qkv.dtype not in (ms.float16, ms.bfloat16)
    if convert_dtype:
...
MODERNBERT_ATTENTION_FUNCTION = {
    "flash_attention_2": flash_attention_construct,
...
class ModernBertAttention(nn.Cell):
    ...
    See `construct` method for additional details.
    """
    ...
    def __init__(self, config: ModernBertConfig, layer_id: Optional[int] = None):
        ...
        self.Wqkv = nn.Dense(config.hidden_size, 3 * self.all_head_size, has_bias=config.attention_bias)
        ...
        self.Wo = nn.Dense(config.hidden_size, config.hidden_size, has_bias=config.attention_bias)
...
class ModernBertModel(ModernBertPreTrainedModel):
    ...
    def _update_attention_mask(self, attention_mask: ms.Tensor, output_attentions: bool) -> ms.Tensor:
        ...
        sliding_window_mask = global_attention_mask.masked_fill(window_mask.logical_not(), np.finfo(self.dtype).min)
...
class ModernBertPredictionHead(nn.Cell):
    def __init__(self, config: ModernBertConfig):
        super().__init__()
        self.config = config
        self.dense = nn.Dense(config.hidden_size, config.hidden_size, config.classifier_bias)
...
class ModernBertForMaskedLM(ModernBertPreTrainedModel):
    ...
    def __init__(self, config: ModernBertConfig):
        ...
        self.decoder = nn.Dense(config.hidden_size, config.vocab_size, has_bias=config.decoder_bias)
    ...
    def set_output_embeddings(self, new_embeddings: nn.Dense):
