...
from ...mindspore_adapter import dtype_to_min, dtype_to_max
...
def load_balancing_loss_func(router_probs: ms.Tensor, expert_indices: ms.Tensor) -> float:
    ...
    expert_mask = mint.max(expert_mask, axis=-2).values

    # cast to float32 otherwise mean will fail
    expert_mask = expert_mask.to(ms.float32)
    tokens_per_group_and_expert = mint.mean(expert_mask, axis=-2)

    router_prob_per_group_and_expert = mint.mean(router_probs, axis=-2)
...
class NllbMoeScaledWordEmbedding(mint.nn.Embedding):
    """
...
class NllbMoeSinusoidalPositionalEmbedding(nn.Cell):
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds, past_key_values_length):
        ...
        return position_ids.unsqueeze(0).broadcast_to(input_shape).contiguous() + past_key_values_length
...
class NllbMoeTop2Router(nn.Cell):
    ...
    def __init__(self, config: NllbMoeConfig):
        ...
        self.dtype = config.router_dtype
    ...
    def normalize_router_probabilities(self, router_probs, top_1_mask, top_2_mask):
        ...
        denom_s = mint.clamp(top_1_max_probs + top_2_max_probs, min=dtype_to_min(router_probs.dtype).eps)
    ...
    def route_tokens(
        ...
        router_probs = mint.nn.functional.softmax(router_logits, dim=-1, dtype=self.dtype).to(input_dtype)
        top_1_expert_index = mint.argmax(router_probs, dim=-1)
        top_1_mask = mint.nn.functional.one_hot(top_1_expert_index, num_classes=self.num_experts)

        if self.second_expert_policy == "sampling":
            gumbel = nn.probability.distribution.Gumbel(0, 1).rsample
        ...
        top_2_expert_index = mint.argmax(logits_except_top_1, dim=-1)
        top_2_mask = mint.nn.functional.one_hot(top_2_expert_index, num_classes=self.num_experts)
        ...
        if self.second_expert_policy == "random":
            top_2_max_probs = (router_probs * top_2_mask).sum(dim=1)
            sampled = (2 * top_2_max_probs) > mint.rand_like(top_2_max_probs.float())
        ...
        if self.batch_prioritized_routing:
            ...
            sorted_cumsum1 = (mint.cumsum(sorted_top_1_mask, dim=0) - 1) * sorted_top_1_mask
            ...
            sorted_cumsum2 = (mint.cumsum(sorted_top_2_mask, dim=0) - 1) * sorted_top_2_mask
            locations2 = sorted_cumsum2[importance_scores.argsort(dim=0).argsort(dim=0)]
            # Update 2nd's location by accounting for locations of 1st
            locations2 += mint.sum(top_1_mask, dim=0, keepdim=True)

        else:
            locations1 = mint.cumsum(top_1_mask, dim=0) - 1
            locations2 = mint.cumsum(top_2_mask, dim=0) - 1
            # Update 2nd's location by accounting for locations of 1st
            locations2 += mint.sum(top_1_mask, dim=0, keepdim=True)
        ...
        top_1_mask = top_1_mask * mint.lt(locations1, self.expert_capacity)
        top_2_mask = top_2_mask * mint.lt(locations2, self.expert_capacity)
    ...
    def construct(self, hidden_states: ms.Tensor, padding_mask: Optional[ms.Tensor] = None) -> tuple:
        r"""
        ...
        Returns:
            top_1_mask (`ms.Tensor` of shape (batch_size, sequence_length)):
            ...
            router_probabilities (`ms.Tensor` of shape (batch_size, sequence_length, nump_experts)):
                Tensor of shape (batch_size, sequence_length, num_experts) corresponding to the probabilities for each
                token and expert. Used for routing tokens to experts.
            router_logits (`ms.Tensor` of shape (batch_size, sequence_length))):
...
class NllbMoeDenseActDense(nn.Cell):
    def __init__(self, config: NllbMoeConfig, ffn_dim: int):
        super().__init__()
        self.fc1 = mint.nn.Linear(config.d_model, ffn_dim)
        self.fc2 = mint.nn.Linear(ffn_dim, config.d_model)
...
class NllbMoeSparseMLP(nn.Cell):
    ...
    def __init__(self, config: NllbMoeConfig, ffn_dim: int, expert_class: nn.Cell = NllbMoeDenseActDense):
        super().__init__()
        self.router = NllbMoeTop2Router(config)
        self.moe_token_dropout = config.moe_token_dropout
        self.token_dropout = mint.nn.Dropout(p=self.moe_token_dropout)
        self.num_experts = config.num_experts
    ...
    def construct(self, hidden_states: ms.Tensor, padding_mask: Optional[ms.Tensor] = False):
        r"""
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_dim)`):
                The hidden states
            padding_mask (`ms.Tensor`, *optional*, defaults to `False`):
                Attention mask. Can be in the causal form or not.

        Returns:
            hidden_states (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_dim)`):
                Updated hidden states
            router_logits (`ms.Tensor` of shape `(batch_size, sequence_length, num_experts)`):
        ...
        hidden_states = hidden_states.reshape((batch_size * sequence_length), hidden_dim)
        masked_hidden_states = mint.einsum("bm,be->ebm", hidden_states, router_mask)
        for idx, expert in enumerate(self.experts.values()):
            ...
            masked_hidden_states[idx, token_indices] = mint.einsum("b,be->be", combining_weights, expert_output)
        hidden_states = masked_hidden_states.sum(dim=0).reshape(batch_size, sequence_length, hidden_dim)

        top_1_expert_index = mint.argmax(top_1_mask, dim=-1)
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights = mint.matmul(query, key.swapaxes(2, 3)) * scaling
...
class NllbMoeAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # get query proj
        query_states = self.q_proj(hidden_states).view(*q_input_shape).swapaxes(1, 2)
        ...
        if (
            ...
            # cross_attentions
            key_states = self.k_proj(encoder_hidden_states).view(*kv_input_shape).swapaxes(1, 2)
            value_states = self.v_proj(encoder_hidden_states).view(*kv_input_shape).swapaxes(1, 2)
        elif past_key_value is not None:
            # reuse k, v, self_attention
            key_states = self.k_proj(hidden_states).view(*kv_input_shape).swapaxes(1, 2)
            value_states = self.v_proj(hidden_states).view(*kv_input_shape).swapaxes(1, 2)
...
class NllbMoeEncoderLayer(nn.Cell):
    def __init__(self, config: NllbMoeConfig, is_sparse: bool = False):
        ...
        self.ff_layer_norm = mint.nn.LayerNorm(config.d_model)
        self.ff_dropout = mint.nn.Dropout(p=config.activation_dropout)
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16 and (
            mint.isinf(hidden_states).any() or ops.isnan(hidden_states).any()
        ):
...
class NllbMoeEncoder(NllbMoePreTrainedModel):
    ...
    Args:
        config:
            NllbMoeConfig
        embed_tokens (mint.nn.Embedding):
            output embedding
    """

    def __init__(self, config: NllbMoeConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
...
class NllbMoeForConditionalGeneration(NllbMoePreTrainedModel, GenerationMixin):
    ...
    def _unpack_router_logits(self, router_outputs):
        ...
        total_router_logits = mint.cat(total_router_logits, dim=1) if len(total_router_logits) > 0 else None
        total_expert_indexes = mint.stack(total_expert_indexes, dim=1) if len(total_expert_indexes) > 0 else None
