...
# distributed undzer the License is distributed on an "AS IS" BASIS,
...
class CpmAntLayerNorm(nn.Cell):
    ...
    def __init__(self, config: CpmAntConfig):
        ...
        self.weight = ms.Parameter(mint.empty(config.hidden_size))

    def construct(self, hidden_states: ms.Tensor):
        """
        Args:
            hidden_states (`ms.Tensor` of shape `(batch, seq_len, dim_in)`)
        """
        if hidden_states.shape[-1] != self.dim_norm:
            raise AssertionError("hidden_states.shape[-1] != self.dim_norm")
        old_dtype = hidden_states.dtype
        variance = hidden_states.to(ms.float32).pow(2).mean(dim=-1, keepdim=True)
        hidden_states = (hidden_states * mint.rsqrt(variance + self.eps)).to(old_dtype) * self.weight
...
class CpmAntAttention(nn.Cell):
    def __init__(self, config: CpmAntConfig):
        ...
        self.project_q = mint.nn.Linear(self.dim_model, self.num_heads * self.dim_head, bias=False)
        self.project_k = mint.nn.Linear(self.dim_model, self.num_heads * self.dim_head, bias=False)
        self.project_v = mint.nn.Linear(self.dim_model, self.num_heads * self.dim_head, bias=False)

        self.attention_out = mint.nn.Linear(self.num_heads * self.dim_head, self.dim_model, bias=False)

        self.softmax = mint.nn.Softmax(dim=-1)

        if config.dropout_p is not None:
            self.dropout = mint.nn.Dropout(p=config.dropout_p)
        else:
            self.dropout = None
    ...
    def construct(
        ...
        Args:
            ...
            hidden_kv (`ms.Tensor` of shape `(batch, len_k, dim_model)`)):
                Tensor *key_value* and *query* of shape `(batch, len_k, dim_model)`
            attention_mask (`ms.Tensor` of shape `(batch, len_seq, len_seq)`):
                Avoid invalid areas to participate in the calculation of self-attention.
            position_bias (`ms.Tensor` of shape `(batch, len_seq, len_seq)`):
                Provide positional information to self-attention block.
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers.
            past_key_values (`tuple[ms.Tensor, ms.Tensor]`, *optional*):
        ...
        if past_key_values is not None:
            key = mint.cat([past_key_values[0], key], dim=-2)
            value = mint.cat([past_key_values[1], value], dim=-2)
            len_k = key.shape[-2]

        # (batch_size, num_heads, len_q, dim_head) @ (batch_size, num_heads, dim_head, len_k) -> (batch_size, num_heads, len_q, len_k)
        score = mint.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.dim_head)
        score = score + position_bias

        score = score.masked_fill(
            attention_mask.view(batch_size, 1, len_q, len_k) == ms.tensor(False),
            ms.tensor(float("-inf"), dtype=score.dtype),
        )
        score = self.softmax(score)

        score = score.masked_fill(
            attention_mask.view(batch_size, 1, len_q, len_k) == ms.tensor(False),
            ms.tensor(0, dtype=score.dtype),
        )
...
class CpmAntSelfAttentionBlock(nn.Cell):
    def __init__(self, config: CpmAntConfig):
        ...
        if config.dropout_p:
            self.dropout = mint.nn.Dropout(config.dropout_p)
        else:
            self.dropout = None
    ...
    def construct(
        ...
        """
        Args:
            hidden_states (`ms.Tensor` of shape `(batch, len_seq, dim_model)`):
                Input of transformer block(self-attention block). It can be the raw embedding of a batch of sequences.
            attention_mask (`ms.Tensor` of shape `(batch, len_seq, len_seq)`):
                Avoid invalid areas to participate in the calculation of self-attention.
            position_bias (`ms.Tensor` of shape `(batch, len_seq, len_seq)`):
            ...
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers.
            past_key_values (`Tuple(ms.Tensor)`, *optional*):
                Cached past key and value projection states.
            use_cache (`bool`, *optional*):
                If set to `True`, `past_key_values` key value states are returned and can be used to speed up decoding
                (see `past_key_values`).
...
class CpmAntDenseGatedACT(nn.Cell):
    def __init__(self, config: CpmAntConfig):
        super().__init__()
        self.w_0 = mint.nn.Linear(config.hidden_size, config.dim_ff, bias=False)
        self.w_1 = mint.nn.Linear(config.hidden_size, config.dim_ff, bias=False)
...
class CpmAntFeedForward(nn.Cell):
    def __init__(self, config: CpmAntConfig):
        ...
        self.w_out = mint.nn.Linear(config.dim_ff, config.hidden_size, bias=False)
...
class CpmAntSegmentPositionEmbedding(nn.Cell):
    def __init__(self, config: CpmAntConfig):
        ...
        self.relative_attention_bias = ms.Parameter(
            mint.empty(
    ...
    def construct(
        ...
        key_pos: ms.Tensor,
        query_pos: ms.Tensor,
        key_segment: ms.Tensor,
        query_segment: ms.Tensor,
    ):
        ...
        absolute_position_bucket = self._position_bucket(
                mint.arange(keylen, dtype=ms.int32)[None, :]
                - mint.arange(querylen, dtype=ms.int32)[:, None],
                num_buckets=self.num_buckets,
                max_distance=self.max_distance,
            )
        relative_position_bucket = mint.where(
    ...
    def _position_bucket(self, relative_position, num_buckets=32, max_distance=128):
        ...
        num_buckets //= 2
        relative_buckets = (relative_position > 0).to(ms.int32) * num_buckets
        ...
        relative_postion_if_large = mint.min(
            relative_postion_if_large,
            mint.full_like(relative_postion_if_large, num_buckets - 1),
        )
        relative_buckets += mint.where(is_small, relative_position.to(ms.int32), relative_postion_if_large)
...
class CpmAntModel(CpmAntPreTrainedModel):
    def __init__(self, config: CpmAntConfig):
        ...
        self.segment_embedding = mint.nn.Embedding(config.segment_types, config.hidden_size)
        self.input_embedding = mint.nn.Embedding(
    ...
    def _prepare_attention_mask(self, input_ids, span, context, length):
        ...
        directional_mask_2d = mint.arange(seqlen) <= mint.arange(seqlen).view(-1, 1)
        ...
        mask_1d = (
            ms.tensor(list(range(seqlen - self.prompt_length))[::-1])[None, :].repeat(batch, 1)
            < length[:, None]
        )
        mask_1d = mint.cat((mint.ones((batch, self.prompt_length)).bool(), mask_1d), dim=1)
    ...
    def construct(
        ...
        **kwargs,
    ) -> Union[tuple[ms.Tensor], BaseModelOutputWithPast]:
        r"""
        input_ids (`ms.Tensor` of shape `(batch_size, seq_len)`):
        ...
        # add prompts ahead
        if input_ids.dtype != ms.int32:
            input_ids = input_ids.to(ms.int32)
        dtype = input_ids.dtype
        segment = mint.where(input_ids != 0, 2, 0).to(dtype=dtype)
        ...
        segment = mint.cat((mint.zeros((batch, self.prompt_length), dtype=dtype), segment), dim=1)
        context = mint.full((batch, seq_length), 1, dtype=dtype)
        position = mint.arange(seq_length, dtype=dtype).repeat(batch, 1)
        span = mint.full((batch, seq_length), 0, dtype=dtype)
...
class CpmAntForCausalLM(CpmAntPreTrainedModel, GenerationMixin):
    ...
    def construct(
        self,
        input_ids: Optional[ms.Tensor] = None,
        past_key_values: Optional[list[tuple[ms.Tensor, ms.Tensor]]] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        labels: Optional[ms.Tensor] = None,
        return_dict: Optional[bool] = None,
        attention_mask: Optional[ms.Tensor] = None,  # dummy parameter for text-generation pipeline
        **kwargs,
