...
class BartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def construct(self, input_ids: ms.Tensor, past_key_values_length: int = 0):
        ...
        positions = mint.arange(
            past_key_values_length, past_key_values_length + seq_len, dtype=ms.int32
        ).view(bsz, -1)
...
class BartEncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        hidden_states = nn.Dropout(p=self.activation_dropout)(hidden_states)
...
class BartModel(BartPreTrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        embed_scale = math.sqrt(config.d_model) if config.scale_embedding else 1.0
        self.shared = BartScaledWordEmbedding(vocab_size, config.d_model, padding_idx=padding_idx, embed_scale=embed_scale)
    ...
    def _tie_weights(self):
        if self.config.tie_word_embeddings:
            ...
            if self.shared.embedding_table.device == "meta" \
