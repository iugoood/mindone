...
class BertLMHeadModel(BertPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **loss_kwargs,
        ...
        lm_loss = None
        if labels is not None:
            lm_loss = self.loss_function(prediction_scores, labels.int(), self.config.vocab_size, **loss_kwargs)
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        input_ids (`ms.Tensor` of shape `(batch_size, num_choices, sequence_length)`):
        ...
        token_type_ids (`ms.Tensor` of shape `(batch_size, num_choices, sequence_length)`, *optional*):
        ...
        position_ids (`ms.Tensor` of shape `(batch_size, num_choices, sequence_length)`, *optional*):
            Indices of positions of each input sequence tokens in the position embeddings. Selected in the range `[0,
            config.max_position_embeddings - 1]`.

            [What are position IDs?](../glossary#position-ids)
        inputs_embeds (`ms.Tensor` of shape `(batch_size, num_choices, sequence_length, hidden_size)`, *optional*):
