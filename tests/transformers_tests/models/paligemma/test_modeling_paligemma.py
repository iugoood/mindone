...
DTYPE_AND_THRESHOLDS_1 = {"fp32": 5e-4, "fp16": 5e-3, "bf16": 5e-2}
DTYPE_AND_THRESHOLDS_0 = {"fp32": 5e-4, "fp16": 5e-3}
...
class PaliGemmaVisionText2TextModelTester:
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        return config, pixel_values, input_ids, attention_mask, np.array(input_ids, dtype=np.int32), np.zeros_like(input_ids)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for dtype in DTYPE_AND_THRESHOLDS_1.keys()
    ],
)
def test_named_modules_1(
    ...
    THRESHOLD = DTYPE_AND_THRESHOLDS_1[ms_dtype]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for case in PALIGEMMA_CASES
        for dtype in DTYPE_AND_THRESHOLDS_0.keys()
    ],
)
def test_named_modules_0(
        name,
        pt_module,
        ms_module,
        init_args,
    ...
    THRESHOLD = DTYPE_AND_THRESHOLDS_0[ms_dtype]
    assert (np.array(diffs) < THRESHOLD).all(), (
        f"ms_dtype: {ms_dtype}, pt_type:{pt_dtype}, "
        f"Outputs({np.array(diffs).tolist()}) has diff bigger than {THRESHOLD}"
    )
